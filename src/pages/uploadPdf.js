...
const UploadPdf = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/uploadPDF`, {
        method: 'POST',
        body: formData
      });
      ...
      if (response.ok) {
        ...
        // Navigate to the questionModule route with data on success
        navigate('/questionModule', { state: { data: data } });
      } else {
